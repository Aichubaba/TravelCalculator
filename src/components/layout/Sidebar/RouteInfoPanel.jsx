...
import { FaRoute, FaClock, FaBed, FaToggleOn, FaToggleOff, FaChevronDown, FaChevronUp } from 'react-icons/fa';
...
const RouteInfoPanel = ({
  ...
  const travelCalc = routeInfo.distance > 0
    ? calculateTravelTime(routeInfo.distance, transportType)
    : null;
    ...
    <>
      ...
      {routeInfo.distance > 0 && (
        <div className="mb-4 space-y-2">
          ...
          {routeInfo.isFallback && <p className="text-sm text-red-500">{t('approx')}</p>}
      ...
      <div className={`flex items-center justify-between border-t ${borderClass} pt-3 mt-3`}>
        ...
        <button
          onClick={() => setUseTravelTime(!useTravelTime)}
          className="text-purple-600 dark:text-purple-400 hover:text-purple-800"
        >
      ...
      {/* Сворачиваемый порядок маршрута */}
      <div className={`mt-4 border-t ${borderClass} pt-3`}>
        <button
          onClick={() => setShowRouteOrder(!showRouteOrder)}
          className="flex items-center justify-between w-full text-sm font-medium hover:bg-purple-200 dark:hover:bg-gray-700 p-1 rounded"
        >
